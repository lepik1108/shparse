...
#get cat descr if exist
        try:
            ...
            category_descr = addslashes(category_descr)
...
#get manufacturer if exist
                try:
                    ...
                    product_manufacturer = addslashes(product_manufacturer)
...
#get price if exist
                try:
                    product_price = float(bro.find_element_by_css_selector(".price").get_attribute('innerHTML')[18:])
                except:
                    product_price = 1
                print (product_price)
#get retail_price if exist
                try:
                    product_r_price = float(bro.find_element_by_css_selector(".retail").get_attribute('innerHTML')[9:]) 
                except:
                    product_r_price = product_price*2
...
#get desc short if exist
                try:
                    ...
                    product_desc_s = addslashes(product_desc_s)
                except:
                    product_desc_s = ''
...
#get desc full if exist
                try:    
                    ...
                    product_desc_f = addslashes(product_desc_f)
                except:
                    product_desc_f = ''
...
###write pillf
                ...
                pillf_php = pillf.make(product_id,product_name,product_desc_f,product_desc_s,product_manufacturer,product_uri,str(product_price),str(product_r_price))
                f.write(pillf_php)
                f.close()
#go back to cat page
                bro.get(item+"#/?_=1&page="+str(page))
